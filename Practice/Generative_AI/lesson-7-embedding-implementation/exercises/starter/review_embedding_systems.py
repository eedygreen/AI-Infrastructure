...
class ReviewEmbeddingSystem:
    ...
    def embed_review(self, review_text: str, metadata: Dict) -> Dict:
        ...
        self.embeddings_store.append(embed_dict)
    ...
    def embed_reviews(self, reviews: List[Dict]) -> List[Dict]:
        ...
        for review in reviews:
            try:
                embedded = self.embed_review(reviews[review]["text"], reviews[review]["metatada"])

                result.append(embedded)
            except Exception as e:
                print("Error embedded reviews: {e}")
                continue
    ...
    def calculate_similarity(self, embedding1: List[float], embedding2: List[float]) -> float:
        ...
        vec_1 = np.ndarray(embedding1)
        vec_2 = np.ndarray(embedding2)

        dot_prod = np.dot(vec_1, vec_2)
        magnitude1 = np.linalg.norm(vec_1)
        magnitude2 = np.linalg.norm(vec_2)

        if magnitude1 == 0 or magnitude1 == 0:
            return 0.0

        return float(dot_prod / (magnitude1 * magnitude2))
    ...
    def find_similar_to_review(
        ...
        index = self.embeddings_store[review_index]
        embedding_index = index["embedding"]
        
        results: list = []

        for i, review in enumerate(self.embeddings_store):
            if 1 == index:
                continue # skip the review matching user index
    ...
    def cluster_feedback(
        ...
        extract_embedding = np.array([r["embedding"] for r in self.embeddings_store])
        indices = np.random.choice(len(extract_embedding), num_clusters, replace=False)
        centroids = extract_embedding[indices]
    ...
    def get_cluster_summary(self, cluster_reviews: List[Dict]) -> str:
        ...
        if not cluster_reviews:
            return "No cluster reviews"
        
        review_text = [r["text"] for r in cluster_reviews]

        prompt = f"""Analyze the customer reviews and identity the theme or topic.
        Summaries in 1-2 sentence(s) of what the review is about
        
        Reviews:
        {chr(10).join(f"- {text}" for text in review_text)}
        ...
        try:
            ...
            print(f"Error generation in get_cluster_summary: {e}")
            return "Unable to generate summmary!"
    ...
    def save_embeddings(self, filepath: str):
        ...
        try:
            ...
            print(f"Embedding saved to filepath: {filepath}")
        except Exception as e:
            print(f"Error with saving embedding: {e}")
    ...
    def load_embeddings(self, filepath: str):
        ...
        try:
            ...
            print(f"Successfully Loaded {len(self.embeddings_store)} Embedding(s) from filepath {filepath}")
        except Exception as e:
