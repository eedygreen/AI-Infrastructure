...
class MessageTokenizer:
    ...
    def count_message_tokens(self, messages: List[Dict[str, str]]) -> int:
        ...
        for message in messages: 
            ...
            total_token += self.count_tokens(message.get("role", ""))
            total_token += self.count_tokens(message.get("content", ""))
        return total_token
    ...
    def optimize_conversation_history(
        ...
        for message in reversed(conversation_history):
            message_token = self.count_message_tokens([message])

            if current_tokens + message_token <= max_tokens:
                optimised_list.insert(0, message)
                current_tokens += message_token
            else:
                break

        if system_message:
            optimised_list.insert(0, system_message)
    ...
    def analyze_message_cost(self, message: str, expected_response_tokens: int = 150) -> Dict:
        ...
        input_tokens: int = self.count_tokens(message)
        estimated_cost = self.estimate_cost(input_tokens, expected_response_tokens)

        if input_tokens < 50:
            length_category = "short"
            recommendation = "Optimal Message length, low cost per interactions"
        elif input_tokens < 200:
            length_category = "medium"
            recommendation = "Moderate Message length, balanced cost per interactions"
        else:
            length_category = "long"
            recommendation = "Chunky message, consider restructuring/shorten the message"
        
        return {
            "input_tokens": input_tokens,
            "expected_response": expected_response_tokens,
