...
class RecipeCreatorAgent:
    ...
    def create_recipe(self, recipe_request: dict, feedback=None):
        """Generate a recipe meeting the specified requirements, incorporating feedback if available."""
        system_message = """You are innovative and a creative skilled chef known for creating 
        delicious recipes that also meet specific dietary and nutritional 
        targets. Prioritize flavor and general appeal. Follow dietary guidelines, 
        but you can be flexible unless told otherwise. Experiment with different flavours, 
        styles, try fusion of different places, each time you asked it must be something new, 
        follow instructions but be flexible to inent something new, never seen before.
        Try foods that are slightly outside the guidelines given, experiment, try something bold,
        ...
        if feedback is None:
            ...
            system_message = """You are an expert chef specializing in creating recipe that follows strict dietary constraints.
...
class NutritionEvaluatorAgent:
    ...
    def run(self, recipe_request, proposed_recipe):
...
def optimizer_recipe(recipe_request):
    ...
    for attemp in range(MAX_RETRIES):
        ...
        evaluation = evaluator.run(recipe_request, recipe)
        ...
        if evaluation.lower().startswith("approved"):
            ...
            feedback = evaluation
            print(f"\n Some constraints not met. Optimizing recipe...")
...
if __name__ == "__main__":
    ...
    recipe, evaluation, attempts = optimizer_recipe(RECIPE_REQUEST)

    print(f"\nAttempts: {attempts}")
    if "APPROVED" in evaluation:
        print("\n All dietary constraints satisfied!")
    else:
        print("\n Could not statisfy all constraints after maximum retries.")
