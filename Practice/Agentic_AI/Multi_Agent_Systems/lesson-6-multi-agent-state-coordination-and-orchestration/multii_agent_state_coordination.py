...
In this exercise, I extend the Italian Pasta Factory multi-agent system 
...
def add_to_production_queue(
    ...
    # Estimated the delivery date based on production capacity
    ...
    if priority > 1:
        ...
        if priority == 3:
            days_to_complete = 1
        # Rush orders geet priority over normal orders
        elif priority == 2:
            days_to_complete = max(1, days_to_complete / 2)

    # add 1 day for the order
...
def create_custom_pasta_recipe(
    ...
    """
    # Validate ingredients exist in factory_state.inventory
    ...
    # Update factory_state.known_pasta_shapes using update_known_pasta_shapes()
    factory_state.update_known_pasta_shapes()

    # Return success status with recipe details
    return {
        "success": True,
...
def prioritize_order(order_id: str, new_priority: int) -> Dict[str, Any]:
    ...
    """
    # Validate priority level (1, 2, or 3)
    if new_priority not in [1, 2, 3]:
        return {
            "success": False,
            "message": f"Invalid prioritu level: {new_priority}. Must be 1 (normal), 2 (rush), or 3 (emergency)."
        }
    
    # Find the order in factory_state.production_queue
    order_found = False
    for order in factory_state.production_queue:
        if order.order_id == order_id:
            ...
            # Recalculate estimated delivery date based on new priority
            capacity_info = check_production_capacity()
            days_to_complete = capacity_info["days_to_complete_current_queue"]
            ...
            return {
                "success": True,
                "message": f"Order {order_id} priority update from {old_priority} to {new_priority}.",
...
# proper Orchestrator using ToolCallingAgent pattern
class Orchestrator(ToolCallingAgent):
    ...
    def __init__(self, model):
        ...
        # coordination tools that route requests to different agents
        @tool
        def process_order_info(customer_request: str) -> str:
            ...
            """
            # Route this request to the OrderProcessorAgent
        ...
        def manage_inventory(order_details: str) -> str:
            ...
            """
            # Route this request to the InventoryManagerAgent
            return self.inventory_manager.run(f"""
            Order details: "{order_details}"

            Check if we hace enough ingredients for this order:
            1. Use check_pasta_recipe to get the ingredient requirements
            2. Use check_inventory to verify we have sufficient in ingredients
            3. Calculate if we havve enough ingredients to fulfill this order.
            """)
        ...
        def schedule_production(order_info: str, priority: int = 1) -> str:
            ...
            """
            # Route this request to the ProductionManagerAgent
        ...
        def design_custom_pasta(customer_request: str) -> str:
            ...
            """
            # Route this request to the CustomPastaDesignerAgent
            return self.pasta_designer.run(f"""
            The Customer request: "{customer_request}
            ...
            2. Create a custom pasta recipe with appropraite ingredient ratios
            3. Name the pasta appropraitely based on its characteristics
        ...
        super().__init__(
            ...
            name="orchestrator",
            description="""
            You are the Orchestrator for the pasta factory system.
            ...
            1. Use process_order_info to understand wha the customer wants
            2. If it's a custom pasta request, use design_custom_pasta first
            3. Use manage_inventory to check ingredinet availability
            4. Use schedule_production to add to queue and delivery date

            Determine priority level from customer langauge (rush, emergency, urgent = Higher priority).
    ...
    def process_order(self, customer_request: str) -> str:
        ...
        """
        # Check if it's a custom pasta request
        ...
        1. First process the order information to understant what they want
        2. If it's a custom pasta request, design the custom pasta first
        3. Check and manage inventory to ensure we can fulfill the order
        4. Schedule production with appropraite priority and provide delivery information
