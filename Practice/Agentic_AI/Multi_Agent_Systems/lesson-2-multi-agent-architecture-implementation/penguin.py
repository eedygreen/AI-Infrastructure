...
# ========================================
# STUDENT TASK 1: Implement the find_food tool
# ========================================
@tool
def find_food(method: str) -> int:
    """
    Penguin finds food independently. Returns amount found (int).
...
class ScientistAgent(ToolCallingAgent):
    ...
    def refresh_resources(self):
        ...
        self.toy_available = True
        print(f"\n🔄 Scientist Resources Refreshed! Food: {self.food_supply}, Toy: {self.toy_available}")
    ...
    def respond_to_action(self, penguin: 'PenguinAgent', action_details: Dict[str, Any]) -> None:
        ...
        print(f"Scientist Resources (before): Food: {self.food_supply}, Toy: {self.toy_available}")
        ...
        Your resources: Food Supply: {self.food_supply}, Toy Available: {self.toy_available}.

        You can use 'check_history' for {penguin.name} if needed.
        Then, decide food (0-5) and toy (true/false) for {penguin.name}.
...
class PenguinAgent(ToolCallingAgent):
    ...
    def take_action(self) -> Dict[str, Any]:
        ...
        Choose ONE action:
        1. FIND FOOD: Use 'find_food' tool with method="fishing" or method="foraging" You must provide 'penguin_name' (which is "{self.name}").
        After the tool is used by the system, your final response should be simple, like "Okay, I went searching."
        2. REQUEST FOOD: Respond with text asking for food (e.g, "I am hungry and need food."). Do Not use any tool.
        3. REQUEST TOY: Respond with text asking for a toy (e.g, "I could use a toy.") Do Not use any tool.
        """
            ...
            if hasattr(step, 'tool_calls') and step.tool_calls:
                for tc in step.tool_calls:
                    if tc.name == 'find_food':
                        ...
                        if hasattr(step, 'observations') and step.observations is not None:
                            try:
                                food_value_from_tool_output = int(str(step.observations))
        ...
        if find_food_tool_called_by_llm:
            ...
            if self.memory.steps:
                ...
                if hasattr(last_step, 'tool_calls') and last_step.tool_calls and \
                   ...
                   hasattr(last_step, 'action_output'):
...
def run_simulation(num_rounds=2, num_penguins=2):
    ...
    print(f"\n🐧🐟🛠️ Starting Antarctic Agents Simulation ({num_rounds} rounds, {num_penguins} penguins) 🛠️🐟🐧")
