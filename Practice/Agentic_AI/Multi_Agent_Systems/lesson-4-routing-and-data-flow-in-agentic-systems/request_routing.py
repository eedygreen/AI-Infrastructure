...
def analyze_request_urgency(request: str) -> str:
    ...
    """
    eng_urgency = ["urgent", "emergency", "immediately", "asap", "right away", "right now"]
    chin_urgency = ["紧急", "急需", "立即", "马上", "立刻", "赶快", "尽快", "迫切", "急迫"]

    text_lower = request.lower()

    for keyword in eng_urgency:
        if keyword in text_lower:
            return "urgent"

    for keyword in chin_urgency:
...
class ChineseBankPostOfficeAgent(ToolCallingAgent): 
    ...
    def handle_customer_request(self, customer_name: str, request: str, expected_service_for_metric: str) -> str:
        ...
        If the request was assessed as urgent (current assessment: '{urgency_level}'), you MUST pass 'is_urgent' flag (value: {is_urgent_bool}) to the handler tool.
