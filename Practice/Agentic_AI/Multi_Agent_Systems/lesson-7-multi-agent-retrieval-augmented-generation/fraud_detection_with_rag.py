...
from demo.demo import (
    ...
    Database, VectorKnowledgeBase, VectorClaimSearch, ComplaintResolutionOrchestrator,
...
# Create a knowledge base of fraud patterns
fraud_patterns = [
{
        'pattern_id': 'FP-001',
...
# vector-based fraud pattern detector
class FraudPatternDetector:
    ...
    def update_patterns(self, fraud_patterns):
        """Update the patterns database with vector embeddings"""
    ...
    def detect_fraud_indicators(self, claim, patient_history, access_level=PrivacyLevel.AGENT):
        """
        Detect fraud indicators for certain claim using vector similarity and rule-based methods

        Args:
            claim: The claim to analyze
            patient_history: Optional list of previous claims for patient
            access_level: The access level of the requester

        Returns:
            Dictionary with fruad indicators and risk assessment
        """
        # Ensure we have initialized patterns
        if not self.is_initialized:
            return {
                'claim_id': claim.id if hasattr(claim, 'id') else 'unknown',
                'error': 'Fruad pattern detector not initialized',
                'fraud_risk': 'unknown'
            }
    ...
    def _apply_fraud_rules(self, claim, patient_history):
        ...
        # Check for duplicate claim if history available
        if patient_history:
            # Look for claims with some procedure code in recent history
    ...
    def _calculate_fraud_risk(self, pattern_matches, rule_indicators):
        """Calculatpr overall fraud risk based on matches and rules"""
...
class FraudDetectionAgent(ToolCallingAgent):
    ...
    def __init__(self, model: OpenAIServerModel):
        super().__init__(
            ...
            that might help assess the legitmacy of claims."""
        )
        
        self.access_level = PrivacyLevel.AGENT

# STEP 5: Update the orchestrator to include fraud detection
# Modify ComplaintResolutionOrchestrator to include fraud detection in the workflow
class EnhancedOrchestrator(ComplaintResolutionOrchestrator):
    ...
    def __init__(self, model: OpenAIServerModel):
        ...
        def handle_claim_with_fraud_check(claim_data: Dict) -> Dict:
            ...
            Returns:
                Dictionay containing the claim processing result with fraud assessment
            """
            ...
            # Run fraud detection on the new claim
            fraud_result = self.fraud_detector.run(
                f"""
                Analyze claim {claim_id} for potential fraud.

                First, get details abput the claim using the get_claim_details tool.
                Then check for fraud indicators using the check_claim_for_fraud tool.
            ...
            if hasattr(fraud_result, 'tool_calls') and fraud_result.tool_calls:
                for call in fraud_result.tool_calls:
                    if call.name == "check_claim_for_fraud" and fraud_result.tool_calls:
                        fraud_analysis = call.argument['fraud_analysis']
...
def demonstrate_fraud_detection():
    ...
    """
    # a demonstration of the fraud detection feature
    orchestrator = EnhancedOrchestrator(model)

    print("Generating a legitmate claim for processing...")

    legitimate_claim = {
        'patient_id': random.choice(list(database.patients.keys())),
        'service_date': f"2023-{random.randint(1, 12):02d}-{random.randint(1, 28):02d}",
