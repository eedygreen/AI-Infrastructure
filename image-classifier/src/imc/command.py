...
from imc.config import setup_model_data, LossFunction, create_criterion
from imc.utils import Checkpoint, process_image, run_validation, train_model, batch_predict
...
def main(ctx: typer.Context,
    ...
    valid_dataloaders = data['valid_dataloaders']
...
def validate(ctx: typer.Context):
    ...
    console.print(f"[blue]Setting up model and data loaders ...[/blue]")

    console.print(f"[blue]Validating model on test dataset...[/blue]")
    model.eval()
    accuracy, test_loss = run_validation(model, test_dataloaders, criterion, device)
    console.print(f"Validation Accuracy: {accuracy / len(test_dataloaders):.2f}")
    console.print(f"Validation Test Loss: {test_loss / len(test_dataloaders):.3f}")
...
def train(
    ...
    Usage:\n
        python -m imc train --epochs 5 --learning-rate 0.001 \n
        python -m imc train --epochs <epochs> --print_every <print_every>
    ...
    ctx.obj["gpu"] = gpu
    console.print("Setting up model and data loaders ...")
...
def save(
    ...
    Usage:\n
        python -m imc save --save-dir <save_dir>\n
        python -m imc save -s <save_dir>
    """
    ctx.ensure_object(dict)
    ...
    console.print(f"[green]✓ Checkpoint Saved Succefully![/green]")
...
def process(
    ...
    console.print(f"[green]Processed {len(dataloader.dataset)} images into DataLoader.[/green]")
    console.print(f"[green]mappings: {class_to_idx}[/green]")
...
def predict(
    ...
    Usage:\n
        ...
        python -m imc predict --image-path <"path_to_images"> --model-path <path_to_model_checkpoint> --topk 3 --label-names cat_to_name.json \n
    """
    ctx.ensure_object(dict)
