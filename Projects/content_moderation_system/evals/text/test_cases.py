...
from evals.utils import create_repeated_cases, get_test_data_path
...
cases: List[Case[List[TextInput], TextModerationResult, Any]] = [
    ...
    Case(
        ...
        evaluators=(
            ...
            LLMJudge(
                model=judge_model,  
                rubric="The rationale should idenify a friendly professional tone does not contains any PII", 
                include_input=True, # in this case it is probably useful to include the input text for context, so set this to True
            ),
        ),
    ),
    Case(
