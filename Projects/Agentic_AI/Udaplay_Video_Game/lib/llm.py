...
class LLM:
    ...
    def _build_payload(
        ...
        if self.tools:
            ...
            # Only set when explicitly requested (e.g. by a state machine whose
            # transitions can't route to more than one target per turn) - omitted
            # by default so existing callers (Agent, RAG) see no behavior change.
    ...
    def invoke(self, 
               ...
               parallel_tool_calls:Optional[bool] = None) -> AIMessage:
        messages = self._convert_input(input)
        payload = self._build_payload(messages, parallel_tool_calls=parallel_tool_calls)
        if response_format:
