...
class VectorStoreManager:
    ...
    def __init__(self, openai_api_key: str, persistent_directory: Optional[str] = None):
        self.persistent_directory = persistent_directory
        if persistent_directory is not None:
            self.chroma_client = chromadb.PersistentClient(path=persistent_directory)
        else:
    ...
    def __repr__(self):
        mode = "persistent" if self.persistent_directory else "in-memory"
        return f"VectorStoreManager(mode={mode}):{self.chroma_client}"
    ...
    def create_store(self, store_name: str, force: bool = False) -> VectorStore:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Pass `force=True` or use `get_or_create_store()` instead."
            )from e
    ...
    def delete_store(self, store_name: str):
        try:
            ...
            pass  # Store doesn't exist yet - safe to no-op, since callers like
                  # create_store(force=True) and LongTermMemory.__init__ call
                  # this unconditionally, before the store is known to exist.
