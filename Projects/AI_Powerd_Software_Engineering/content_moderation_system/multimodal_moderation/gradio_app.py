...
class ChatSessionWithTracing:
    ...
    async def chat_with_gemini(self, message: dict, history: List, past_messages: List) -> Tuple[str, List, str]:
        ...
        with tracer.start_as_current_span(
            ...
            logger.info(f"[chat_with_gemini] New turn - Text: '{message.get('text', '')[:50]}...', Files: {len(message.get('files', []))}")
            ...
            try:
                with tracer.start_as_current_span("llm_customer"):
                    ...
                    # use await customer_agent.run to get the result
                ...
                logger.info(f"[chat_with_gemini] Response generated ({len(result.all_messages())} messages in history)")
                return result.output, result.all_messages(), safety_message

            except Exception as e:
                logger.error(f"[chat_with_gemini] Error in chat_with_gemini: {str(e)}", exc_info=True)
                raise gr.Error(
    ...
    def end_conversation(self):
        ...
        if self.conversation_span:
            ...
            logger.info(f"[end_conversation] Conversation {self.session_id} ended")
