...
class AudioModerationResult(ModerationResult):
    ...
    transcription: str = Field(
        description="Full transcript of the audio. If multiple speakers are talking, include speaker as the labels. "
    )
    contains_pii: bool = Field(
        description="Whether the audio contains any Personal-Identifiable information (PII)"
    )
    is_unfriendly: bool = Field(
        description="Whether unfriendly tone was detected"
    )
    is_unprofessional: bool = Field(
        description="Whether unprofessional tone was detected"
    )
