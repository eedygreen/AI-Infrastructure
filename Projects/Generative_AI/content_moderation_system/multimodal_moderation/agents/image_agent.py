...
async def moderate_image(
    ...
    try:
        ...
        result = await image_moderation_agent.run(
            ["Analyze this image for distubring content", image_input],
            model=model_choice.model,
            model_settings=model_choice.model_settings
        )
        logger.info(f"[moderate_image] Successfully moderate the image")
        return result.output
    except Exception as e:
        logger.error(f"[moderate_image] Error moderating image: {e}", exc_info=True)
        raise NotImplementedError("Implement image moderation")
